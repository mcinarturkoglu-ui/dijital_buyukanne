    ...
    <section className="py-20 md:py-28 px-4 md:px-8 bg-white">
      <div className="max-w-6xl mx-auto">
            ...
            {steps.map((step, index) => (
              <div key={step.number} className="flex items-start flex-1">
                ...
                <div className="flex flex-col items-center gap-3 flex-1">
                  {/* Circle */}
                  <div
                    className={`w-14 h-14 rounded-full ${step.color} ring-4 ${step.ringColor} flex items-center justify-center shadow-md flex-shrink-0`}
                  >
                  ...
                  {/* Title */}
                  <p
                    className={`text-xs font-semibold text-center leading-tight max-w-[80px] ${step.textColor}`}
                  >
                    {step.title}
                  </p>
                </div>
        ...
        <div className="mt-8 hidden md:flex items-center gap-0 px-2">
          {steps.map((step, index) => (
            <div key={step.number} className="flex items-center flex-1">
              <div className={`h-1 flex-1 ${step.color} opacity-40 ${index === 0 ? 'rounded-l-full' : ''} ${index === steps.length - 1 ? 'rounded-r-full' : ''}`} />
            </div>
          ))}
        </div>
        ...
        <div className="mt-14 max-w-2xl mx-auto">
          <blockquote className="relative border-l-4 border-turquoise pl-6 py-2">
            <div className="text-5xl text-turquoise/20 font-serif absolute -top-2 left-2 select-none">&ldquo;</div>
            <p className="text-gray-700 text-lg leading-relaxed italic font-medium relative z-10">
